...
import { createSignal as createSignal$1, createEffect as createEffect$1 } from 'solid-js';
...
const _tmpl$ = template(`<canvas></canvas>`, 2);

const deepClone = config => JSON.parse(JSON.stringify(config));

function SolidChart(props) {
  ...
  createEffect$1(() => {
    ...
    lodash_merge(_chart.config, deepClone(props));
  ...
  return (() => {
    ...
    const _ref$ = createChart;
    typeof _ref$ === "function" ? _ref$(_el$) : createChart = _el$;

    spread(_el$, () => props.canvasOptions, false, false);
